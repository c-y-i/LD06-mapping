#include "LD06_LiDAR.h"

#include <math.h>

namespace
{

constexpr float kSensorYawOffsetDeg = 0.0f;
constexpr bool kMirrorScanYAxis = true;

constexpr uint8_t kCrcTable[256] = {
    };

} // namespace

LD06_LiDAR::LD06_LiDAR(HardwareSerial &serial_port) : serial_(serial_port)
{
  clearScanFrame(scan_buffers_[0]);
  clearScanFrame(scan_buffers_[1]);
}

void LD06_LiDAR::begin(uint32_t baud_rate, int rx_pin, int tx_pin)
{
  serial_.begin(baud_rate, SERIAL_8N1, rx_pin, tx_pin);
}


uint16_t LD06_LiDAR::readLeU16(const uint8_t *data)
{
  return static_cast<uint16_t>(data[0]) |
         (static_cast<uint16_t>(data[1]) << 8);
}

float LD06_LiDAR::normalizeAngle(float angle_deg)
{
  while (angle_deg < 0.0f)
  {
    angle_deg += 360.0f;
  }
  while (angle_deg >= 360.0f)
  {
    angle_deg -= 360.0f;
  }
  return angle_deg;
}

bool LD06_LiDAR::decodePacket(const uint8_t *packet, LD06PacketSummary &summary)
{
  if (packet[0] != kLD06PacketHeader || packet[1] != kLD06PacketLength)
  {
    return false;
  }

  summary.speed_raw = readLeU16(packet + 2);
  summary.start_angle_deg = readLeU16(packet + 4) / 100.0f;
  summary.end_angle_deg = readLeU16(packet + 42) / 100.0f;
  summary.timestamp_ms = readLeU16(packet + 44);
  summary.valid_points = 0;

  float span_deg = summary.end_angle_deg - summary.start_angle_deg;
  if (span_deg < 0.0f)
  {
    span_deg += 360.0f;
  }

  const float step_deg = span_deg / static_cast<float>(kLD06PointsPerPacket);
  if (step_deg <= 0.0f || step_deg > kLD06MaxAngleStepDeg)
  {
    return false;
  }

  for (size_t i = 0; i < kLD06PointsPerPacket; ++i)
  {
    const size_t offset = 6 + (i * 3);
    const uint16_t distance_mm = readLeU16(packet + offset);
    const uint8_t intensity = packet[offset + 2];
    const float physical_angle_deg =
        normalizeAngle(summary.start_angle_deg +
                       ((static_cast<float>(i) + 0.5f) * step_deg));
    const float angle_deg =
        normalizeAngle(360.0f - physical_angle_deg + kSensorYawOffsetDeg);
    const float angle_rad = angle_deg * DEG_TO_RAD;

    LD06ScanPoint point;
    point.angle_deg = angle_deg;
    point.distance_mm = distance_mm;
    point.intensity = intensity;
    point.x_mm = static_cast<int16_t>(lroundf(distance_mm * cosf(angle_rad)));
    point.y_mm = static_cast<int16_t>(
        lroundf((kMirrorScanYAxis ? 1.0f : -1.0f) * distance_mm * sinf(angle_rad)));
    point.valid = distance_mm > 0;
    summary.points[i] = point;

    if (point.valid)
    {
      ++summary.valid_points;
    }
  }

  return true;
}

void LD06_LiDAR::clearScanFrame(LD06ScanFrame &frame)
{
  frame.speed_raw = 0;
  frame.timestamp_ms = 0;
  frame.point_count = 0;
  frame.valid_point_count = 0;
  frame.crc_fail_count = crc_fail_count_;
}

void LD06_LiDAR::appendPointToCurrentScan(const LD06PacketSummary &summary,
                                          const LD06ScanPoint &point)
{
  current_scan_->speed_raw = summary.speed_raw;
  current_scan_->timestamp_ms = summary.timestamp_ms;
  current_scan_->crc_fail_count = crc_fail_count_;

  if (current_scan_->point_count >= kLD06MaxPointsPerScan)
  {
    return;
  }

  current_scan_->points[current_scan_->point_count++] = point;
  if (point.valid)
  {
    ++current_scan_->valid_point_count;
  }
}

void LD06_LiDAR::publishCurrentScan()
{
  latest_scan_ = current_scan_;
  latest_scan_->crc_fail_count = crc_fail_count_;

  current_scan_ =
      (current_scan_ == &scan_buffers_[0]) ? &scan_buffers_[1] : &scan_buffers_[0];
  clearScanFrame(*current_scan_);
}

bool LD06_LiDAR::processPacket(const LD06PacketSummary &summary)
{
  bool new_scan_ready = false;

  for (size_t i = 0; i < kLD06PointsPerPacket; ++i)
  {
    const LD06ScanPoint &point = summary.points[i];
    const float physical_angle_deg = normalizeAngle(360.0f - point.angle_deg);

    if (!have_last_physical_angle_)
    {
      have_last_physical_angle_ = true;
      last_physical_angle_deg_ = physical_angle_deg;
      start_physical_angle_deg_ = physical_angle_deg;
    }

    if (physical_angle_deg < last_physical_angle_deg_)
    {
      if (!scan_initialized_)
      {
        scan_initialized_ = true;
        clearScanFrame(*current_scan_);
      }
      else if ((last_physical_angle_deg_ - start_physical_angle_deg_) > 340.0f &&
               current_scan_->point_count > 0)
      {
        publishCurrentScan();
        new_scan_ready = true;
      }

      start_physical_angle_deg_ = physical_angle_deg;
    }

    last_physical_angle_deg_ = physical_angle_deg;

    if (!scan_initialized_)
    {
      continue;
    }

    appendPointToCurrentScan(summary, point);
  }

  return new_scan_ready;
}

bool LD06_LiDAR::update()
{
  bool new_scan_ready = false;

  while (serial_.available() > 0)
  {
    const uint8_t byte_in = static_cast<uint8_t>(serial_.read());

    if (packet_index_ == 0)
    {
      if (byte_in != kLD06PacketHeader)
      {
        continue;
      }

      packet_buffer_[packet_index_] = byte_in;
      computed_crc_ = kCrcTable[byte_in];
      ++packet_index_;
      continue;
    }

    if (packet_index_ == 1 && byte_in != kLD06PacketLength)
    {
      packet_index_ = 0;
      computed_crc_ = 0;
      if (byte_in == kLD06PacketHeader)
      {
        packet_buffer_[packet_index_] = byte_in;
        computed_crc_ = kCrcTable[byte_in];
        ++packet_index_;
      }
      continue;
    }

    packet_buffer_[packet_index_] = byte_in;

    if (packet_index_ < (kLD06PacketSize - 1))
    {
      computed_crc_ = kCrcTable[computed_crc_ ^ byte_in];
      ++packet_index_;
      continue;
    }

    const uint8_t expected_crc = computed_crc_;
    packet_index_ = 0;
    computed_crc_ = 0;

    if (expected_crc != byte_in)
    {
      ++crc_fail_count_;
      continue;
    }

    LD06PacketSummary summary;
    if (decodePacket(packet_buffer_, summary))
    {
      last_packet_ = summary;
      ++packets_seen_;
      if (processPacket(summary))
      {
        new_scan_ready = true;
      }
    }
  }

  return new_scan_ready;
}

void LD06_LiDAR::printPacketSummary(Stream &stream) const
{
  stream.printf("packets=%lu speed=%u start=%.2f end=%.2f valid=%u ts=%u crc_fail=%lu\n",
                static_cast<unsigned long>(packets_seen_),
                last_packet_.speed_raw,
                last_packet_.start_angle_deg,
                last_packet_.end_angle_deg,
                static_cast<unsigned>(last_packet_.valid_points),
                last_packet_.timestamp_ms,
                static_cast<unsigned long>(crc_fail_count_));

  for (size_t i = 0; i < kLD06PointsPerPacket; ++i)
  {
    const LD06ScanPoint &point = last_packet_.points[i];
    if (!point.valid)
    {
      continue;
    }

    stream.printf("  angle=%7.2f deg  dist=%5u mm  intensity=%3u  x=%6d  y=%6d\n",
                  point.angle_deg,
                  point.distance_mm,
                  point.intensity,
                  point.x_mm,
                  point.y_mm);
  }
}

const LD06PacketSummary &LD06_LiDAR::lastPacket() const
{
  return last_packet_;
}

const LD06ScanFrame &LD06_LiDAR::latestScan() const
{
  return *latest_scan_;
}

uint32_t LD06_LiDAR::packetsSeen() const
{
  return packets_seen_;
}

uint32_t LD06_LiDAR::crcFailCount() const
{
  return crc_fail_count_;
}
